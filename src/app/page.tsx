...
const FEATURES = [
  {
    title: "Sleeps 12",
    body: "3 bedrooms, 3 baths, open living facing the pond.",
  },
...
export default function HomeOverhaul() {
  ...
  return (
    <main className="bg-[var(--bg)] text-[var(--fg)]">
      ...
      <div
        aria-hidden
        className="pointer-events-none fixed inset-0 -z-10 mix-blend-soft-light opacity-[0.08]"
        style={{
          backgroundImage:
            "url('data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%224%22 height=%224%22><rect width=%224%22 height=%224%22 fill=%22%23fff%22/><circle cx=%220.5%22 cy=%220.5%22 r=%220.5%22 fill=%22%23000%22/></svg>')",
        }}
      />
      ...
      >
        ...
        <div
          ...
          style={{
            transform: motionOK()
              ? `scale(${titleScale}) translateY(${scrollY / 12}px)`
              : undefined,
        ...
        >
          ...
          <p className="mt-3 text-xl tracking-wide text-[var(--bg)]/90">
            Events & Lodging in Anna, Texas
          </p>
          <p className="mt-2 text-sm text-[var(--bg)]/85">
            20 acres • Private pond • Sleeps 12 • Event-friendly
          </p>
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row">
            ...
            <a
              href="#events"
              className="rounded-full border border-[color:var(--bg)/0.7] px-6 py-3 font-medium text-[var(--bg)] hover:bg-[color:var(--bg)/0.08]"
            >
              Event Packages
            </a>
          </div>
      ...
      {/* VALUE STRIP */}
      <section className="border-b border-[color:var(--fg)/0.08] bg-[color:var(--panel)/0.08]">
        <div className="mx-auto max-w-6xl px-4 py-4 flex flex-wrap gap-2">
          {[
            "Whole house · 3 bed · 3 bath",
            "Sleeps 12",
            "Pond-side event lawn",
            "Direct inquiries",
          ].map((t) => (
            <span
              key={t}
              className="rounded-full border border-[color:var(--fg)/0.15] bg-[color:var(--bg)]/6 px-3 py-1.5 text-sm text-[color:var(--fg)/0.9]"
            >
              {t}
            </span>
          ))}
      ...
      <section id="choose" className="mx-auto max-w-6xl px-4 py-10">
        <div className="grid gap-6 md:grid-cols-2">
          <a
            href="#stay-details"
            className="group relative overflow-hidden rounded-3xl border border-[color:var(--fg)/0.15] p-6 bg-gradient-to-br from-[color:var(--panel)/0.14] to-[color:var(--panel)/0.06]"
          >
            <div className="pointer-events-none absolute -left-10 top-1/3 h-56 w-56 rounded-full bg-[var(--accent)]/15 blur-2xl transition-transform duration-300 group-hover:scale-110" />
            <h3 className="text-2xl font-semibold">Stay at Starlit Grove</h3>
            <p className="mt-2 text-[color:var(--fg)/0.85]">
              3 bedrooms, 3 baths, open living with pond views, game room, and
              laundry.
            </p>
            <span className="mt-4 inline-block rounded-full bg-[var(--accent)] px-5 py-2 text-sm font-medium text-[var(--fg)]">
              Explore the home
            </span>
          </a>
          ...
          >
            ...
            <p className="mt-2 text-[color:var(--fg)/0.85]">
              Pond-side lawn for tented celebrations, weddings, and reunions.
            </p>
      ...
      <section id="stay-details" className="mx-auto max-w-6xl px-4 py-12">
        <div className="rounded-3xl border border-[color:var(--fg)/0.15] bg-[color:var(--panel)/0.12] p-6">
          ...
          <p className="mt-2 text-[color:var(--fg)/0.85]">
            Modern-country comfort with room for everyone. Fully stocked
            kitchen, open living with pond views, game room, and laundry for
            longer stays.
          </p>
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <div
                key={t}
                className="rounded-2xl border border-[color:var(--fg)/0.12] bg-[color:var(--panel)/0.08] p-4"
              >
                <div className="text-xs uppercase tracking-wide text-[color:var(--fg)/0.65]">
                  {t}
                </div>
      ...
      <section id="packages" className="mx-auto max-w-6xl px-4 py-12">
        ...
        <p className="mt-2 text-[color:var(--fg)/0.85]">
          Choose a simple celebration, a weekend with lodging, or an elegant
          pond-side wedding.
        </p>
        <div className="mt-6 grid gap-5 md:grid-cols-3">
          {PACKAGES.map((p, idx) => (
            ...
            >
              ...
              <span className="mt-5 inline-block rounded-full bg-[var(--accent)] px-4 py-2 text-sm font-medium text-[var(--fg)]">
                View details
              </span>
      ...
      <section id="location" className="mx-auto max-w-6xl px-4 pb-14">
        <div className="rounded-3xl border border-[color:var(--fg)/0.15] bg-[color:var(--panel)/0.12] p-6">
          ...
          <p className="mt-2 text-[color:var(--fg)/0.85]">
            In Anna, Texas, near McKinney and the DFW metro area. Country calm,
            city access.
          </p>
...
function GalleryTile({
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute bottom-0 left-0 right-0 translate-y-2 bg-gradient-to-t from-black/35 via-black/0 to-transparent p-2 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
